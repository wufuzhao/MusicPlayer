...
public class CircleImageView extends ImageView {
	...
	public CircleImageView(Context context, AttributeSet attrs, int defStyle) {
		...
		mBgColor = a.getColor(R.styleable.CircleImageView_bg_color,
				DEFAULT_BG_COLOR);
		a.recycle();
	...
	private void setup() {
		...
		mBorderRadius = Math.min(mBorderRect.height() / 2,
				mBorderRect.width() / 2);
	...
	private void updateShaderMatrix() {
		...
		/*
		 * //去除图片和边框的缝隙 float maxD = Math.max(dx, dy); System.out.println(maxD);
		 * if(maxD>0){ mBorderRadius-=(maxD+0.5f); }
		 */
	}
